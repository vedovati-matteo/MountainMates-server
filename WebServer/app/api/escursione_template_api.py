...
api = Namespace('escursione_template', description='Azioni della classe template')

escursione = api.model('template dell\'escursione completa', {
...
class UtenteSelf(Resource):
    ...
    @api.expect(None, validate = True)
    def get(self):
        """Ottieni i dati del template di un'escursione specifica """
        return get_escursione_self_template(data=request)
